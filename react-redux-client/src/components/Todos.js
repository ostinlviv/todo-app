  ...
  render(){
    ...
    return(
      ...
      {todos.length <= 0 && !todoState.isFetching &&
        <div className="norecordsBox">
        <p className="norecordsTitle"><Glyphicon glyph="leaf" /> There is no records.</p>
        </div>
      }
      ...
      <tbody>
        ...
        <td className="todoTitle"><Glyphicon glyph="hand-right" /><Link className="todo" to={`/${todo._id}`}> {todo.todoText}</Link></td>
        <td className="textCenter"><Button onClick={() => this.showDeleteModal(todo)} bsStyle="info" bsSize="xsmall"><Glyphicon glyph="trash" /></Button></td>
        <td className="textCenter"><Button onClick={() => this.showEditModal(todo)} bsStyle="success" bsSize="xsmall"><Glyphicon glyph="pencil" /> EDIT</Button></td>
        </tr> )
      }
      </tbody>
      </table>
    ...
    <div className="col-md-12">
    {editTodo  &&
    <TodoEditForm todoData={editTodo} editTodo={this.submitEditTodo} hideEditModal={this.hideEditModal} />
    }
